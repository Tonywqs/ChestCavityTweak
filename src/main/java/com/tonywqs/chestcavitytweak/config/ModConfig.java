...
public class ModConfig {
    ...
    /**
     * 获取胸腔背包的行数（4~6）
     * 在配置尚未加载时（如ChestCavity类静态初始化期间）回退到4，防止崩溃
     */
    public static int getCavityRows() {
        try {
            if (CAVITY_ROWS == null) return 4;
            int val = CAVITY_ROWS.get();
            if (val < 4 || val > 6) return 4;
            return val;
        } catch (Exception e) {
            // 配置尚未加载，回退到4行
            return 4;
